...
{
    if (_delegate && [_delegate respondsToSelector:@selector(searchCell:selectedInfo:)]) {
        [_delegate searchCell:self selectedInfo:_info];
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier withMessageInfo:(MessageInfo *)info delegate:(id)delegate_
{
    ...
    if (self) {
        ...
        //时间
        UILabel *time = [Tools createLabel:[NSString stringWithFormat:@"%@", _info.startTime] frame:CGRectMake(125, 22, 200, 15) color:[UIColor whiteColor] font:[UIFont systemFontOfSize:12] aliment:UITextAlignmentLeft];
        ...
        //地点
        UILabel *location = [Tools createLabel:[NSString stringWithFormat:@"%@",_info.dest] frame:CGRectMake(125, 40+titleSize.height+20, 170, 50) color:[UIColor whiteColor] font:[UIFont systemFontOfSize:14] aliment:UITextAlignmentLeft];
