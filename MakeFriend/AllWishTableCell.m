...
-(void)commentabuttonaclicked:(UIButton*)sender
{
    NSLog(@"_info.allCommentInfos:%@",_info.allCommentInfos);
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier message:(AllWishInfo *)info commentCount:(NSString*)count delegate:(id)delegate_
{
 ...
 //布置时间和查看评论按钮
        ...
        commentsButton.frame=CGRectMake(backGroundView.frame.size.width-40, backGroundView.frame.size.height-25, 40, 20);
        commentsButton.backgroundColor=[UIColor clearColor];
        [commentsButton addTarget:self action:@selector(commentabuttonaclicked:) forControlEvents:UIControlEventTouchUpInside];
        [backGroundView addSubview:commentsButton];
  }
    return self;
